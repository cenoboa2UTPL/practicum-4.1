    ...
    private static function proccessAccountPaciente()
    {
        ...
        if(!$Paciente){ /// si no existe paciente 
           ...
           if($response)
           {
            ...
            $modelUpdate = new Usuario;
    
            /// obtenemos al usuario registrado, para actualizar de inmediato el campo codigo_confirm 
    
            $User_Save = $modelUpdate->query()->Where("email","=",self::post("correo"))->first();

              $CodigoAleatorio = DigitsAleatorio();
              $modelUpdate->Update([
                "id_usuario"=>$User_Save->id_usuario,
                "codigo_confirm"=> $CodigoAleatorio
            ]);

            /// enviamos el correo por email
            if( self::send_($User_Save->email,$User_Save->name,"Código de confirmación de la cuenta",self::ContentSendEmailConfirmAccount($CodigoAleatorio)))
            {
                self::Session("success_","Le hemos enviado un código de activación al correo ".$User_Save->email." .El código solo tiene 20 minutos de validez.");
    ...
    private static function sendEmail($correo,$token){
         ...
         if($Usuario)
         {
            if($Usuario->email === $correo){
                ...
                $Tiempo_Expired = time() + (env("TIEMPO_VIDA_SESSION")*5); /// por defecto 5 minutos
