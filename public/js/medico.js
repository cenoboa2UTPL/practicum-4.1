...
function ShowMedicos()
{
  var Medicos_ = $('#tabla-medicos').DataTable({
    ...
    "columns":[
      ...
      {"data":"genero",render:function(data){return data == 1?'<span class="badge bg-primary">MASCULINO</span>':'<span class="bad bg-danger">FEMENINO</span>'}},
      ...
      {"data":"foto",render:function(data_foto){
        if(data_foto == null)
        {
          return `<img src='`+RUTAFOTO+`' style="width: 33px;height: 33px;border-radius:50%">`
        }

        return `<img src='`+RUTAFOTO+data_foto+`'  style="width: 33px;height: 33px;border-radius:50%">`
      }},
      {"defaultContent":`
        <div class="row">
        <div class="col-xl-3 col-lg-3 col-md-4 col-sm-5 col-12 m-2">
        <button class="btn rounded btn-warning btn-sm"><i class='bx bxs-edit-alt'></i></button>
        </div>
        ...
        <div class="col-xl-3 col-lg-3 col-md-4 col-sm-5 col-12 m-2">
        <button class="btn rounded btn-info btn-sm" id='especialidades_asign' ><i class='bx bx-list-check'></i></button>
        </div>
        </div>
      `}
    ]
...
function VerEspecialidadesNoAsignadosDelMedico(idMedico,buscador)
{
  ...
  axios({
    ...
    Data.Especialidades.forEach(especialidad => {
      ...
      <td><span class='badge bg-success'>`+especialidad.precio_especialidad+`</span></td>
      <td>
      <div class='row'>
      ...
      <div class='col-xl-2 col-lg-3 col-12  mx-2 my-xl-0 my-lg-0 my-1'>
      <button class='btn btn-rounded btn-danger btn-sm'><i class='bx bxs-message-square-x' onclick=ConfirmDelete(`+especialidad.id_especialidad+`,'`+especialidad.nombre_esp+`')></i></button>
     </div>
      </div>
      </td>
      </tr>
    ...
    "columns":[
      ...
      {"data":null,render:function(dato){
        ...
        if(dato.estado == 1)
        {
          ...
          <button class="btn rounded btn-warning btn-sm" id='editar-esp'><i class='bx bxs-edit-alt'></i></button>
          </div>
          </div>
          `;
        }
...
function MostrarMedicosEspecialidades_()
{
  var TablaEspecialidadesMedico_ = $('#tabla-medico-programacion-horarios__').DataTable({
    ...
    "columns":[
      ...
      <div class="col-xl-4 col-lg-3 col-md-4 col-sm-4 col-2 m-1">
      <button class="btn rounded btn-primary btn-sm" id='servicio_add' ><i class='bx bxs-cake'></i></button>
      </div>
